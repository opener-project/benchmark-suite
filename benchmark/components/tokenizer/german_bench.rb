...
OpenerBenchmark.benchmark 'tokenizer' do
  ...
  setup do
    ...
    @small_review  = identifier.run(read_fixture('german/small.txt'))
    @medium_review = identifier.run(read_fixture('german/medium.txt'))
    @large_review  = identifier.run(read_fixture('german/large.txt'))
  end
