...
OpenerBenchmark.benchmark 'tokenizer' do
  ...
  setup do
    ...
    @small_review  = identifier.run(read_fixture('french/small.txt'))
    @medium_review = identifier.run(read_fixture('french/medium.txt'))
    @large_review  = identifier.run(read_fixture('french/large.txt'))
  end
