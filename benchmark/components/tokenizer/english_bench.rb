...
OpenerBenchmark.benchmark 'tokenizer' do
  ...
  setup do
    ...
    @small_review  = identifier.run(read_fixture('english/small.txt'))
    @medium_review = identifier.run(read_fixture('english/medium.txt'))
    @large_review  = identifier.run(read_fixture('english/large.txt'))
  end
